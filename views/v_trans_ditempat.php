...
<head>
	...
	<title>DJ Resto | Transaksi Penjualan - Makan Ditempat</title>
			...
			<div class="container-fluid content-top-gap">
				...
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb my-breadcrumb">
						...
						<li class="breadcrumb-item active" aria-current="page">Makan Ditempat</li>
					</ol>
				</nav>
				<!-- End Breadcrumb -->

				<!-- Form Transaksi Penjualan Makan Ditempat -->
						...
						<div class="card-body">
									...
									<div class="row">
										<div class="col-sm-6">
											<div class="form-group mb-4">
												<div class="input-group">
													<div class="input-group-prepend">
														<span class="input-group-text bg-light text-primary"><i class="fa fa-table"></i></span>
													</div>

													<input list="list_meja" name="kd_meja" id="kd_meja" class="form-control input-style" placeholder="Isi Kode Meja" autocomplete="off">
														<datalist id="list_meja">
															...
															?>
																<option value="<?php echo $kd_meja?>"><?php echo $nama_meja?></option>
															<?php } } else { ?>
																<option value="Meja Penuh!">Tidak ada yang kosong!</option>
															<?php } ?>
														</datalist>
													</input>
												</div>
										...
										<div class="col-sm-6">
											<div class="form-group mb-4">
												<div class="input-group">
													<div class="input-group-prepend">
														<span class="input-group-text bg-light text-primary"><i class="fa fa-user"></i></span>
													</div>
							...
							<table class="table table-bordered table-hover mt-4 mb-4">
								...
								<tbody id="tabel_detTransaksi">
									<?php include 'tampil_detTransaksi.php';?>
								</tbody>
							</table>
					...
					<div class="modal-dialog modal-dialog-centered" role="document">
						<div class="modal-content">
							...
							<div class="modal-footer">
								<button type="button" class="btn btn-primary text-light mr-auto btnCNoMeja"><i class="fa fa-print"></i> Cetak No. Meja</button>
	...
	<script type="text/javascript">
		$(document).on('click', '#tambah_detTransaksi', function() {
			...
			if(kd_produk == '') {
				...
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url()?>/tambah_detTransaksi.php',
					...
					success: function() {
						...
						$('#qty').val('');
						$('#kd_produk').focus();
						$('#tabel_detTransaksi').load('<?php echo base_url()?>/tampil_detTransaksi.php', function() {
							calculate();
						});
					}
				});
		...
		$(document).on('click', '#proses_transaksi', function() {
			...
			if(kd_meja == '') {
				...
				Toast.fire({
					icon: 'error',
					title: 'Tolong pilih Kode Meja!'
				})
				$('#kd_meja').focus();
			} else if(kd_meja == 'Meja Penuh!') {
				const Toast = Swal.mixin({
					toast: true,
					position: 'top-end',
					showConfirmButton: false,
				...
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url()?>/tambah_transaksi.php',
					data: ({'proses_transaksi' : true, 'kd_transaksi' : kd_transaksi, 'kd_user' : kd_user, 'kd_meja' : kd_meja, 'nama_pembeli' : nama_pembeli, 'total_bayar' : total_bayar}),
					success: function() {
						$('.modalSimpanTransaksi').modal('show');

						$('.btnCNoMeja').off();
						$('.btnCNoMeja').on('click', function() {
							window.open (
								'<?php echo base_url()?>/cetak_nomeja.php?kd_transaksi=' + kd_transaksi,
								'_blank'
							);
						});
						...
						$('.btnCMenu').on('click', function() {
							window.open (
								'<?php echo base_url()?>/cetak_menuDipesan.php?kd_transaksi=' + kd_transaksi,
								'_blank'
							);
						});
					}

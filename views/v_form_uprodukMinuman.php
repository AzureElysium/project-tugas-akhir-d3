...
<head>
	...
	<title>DJ Resto | Kelola Produk - Ubah Minuman</title>
			...
			<div class="container-fluid content-top-gap">
				...
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb my-breadcrumb">
						...
						<li class="breadcrumb-item"><a href="produk_minuman.php">Minuman</a></li>
						<li class="breadcrumb-item active" aria-current="page">Ubah Minuman</li>
					</ol>
				</nav>
				<!-- End Breadcrumb -->

				<!-- Form Tambah/Ubah Minuman -->
				<div class="accordions pt-3">
					<div class="card card_border border-primary-top">
						...
						<div class="card-body">
							<a href="produk_minuman.php" class="btn btn-warning text-light mb-3"><i class="fa fa-chevron-left"></i>&nbsp;&nbsp;Kembali</a>
							...
							<div class="card card_border border-top shadow p-3">
								<div class="row border-bottom">
									<div class="col-lg-6 border-right">
										<div class="form-group row mb-4">
											...
											<div class="col-sm-8">
												<input type="text" name="kd_produk" id="kd_produk" class="form-control input-style" value="<?php echo @$result->kd_produk?>" readonly>
											</div>
										</div>

										<div class="form-group row mb-4">
											<label class="col-sm-4 col-form-label input__label">Nama Minuman :</label>
											<div class="col-sm-8">
												<input type="text" name="nama_produk" id="nama_produk" class="form-control input-style" placeholder="Isi Nama Minuman" value="<?php echo @$result->nama_produk?>">
											</div>
										</div>
										...
										<div class="form-group row mb-4">
											...
											<div class="col-sm-8">
												<input type="text" name="kategori_produk" id="kategori_produk" class="form-control input-style" value="<?php echo @$result->kategori_produk?>" readonly>
											</div>
										</div>
										...
										<div class="form-group row mb-4">
											...
											<div class="col-sm-8">
												<input type="number" step="1000" name="harga" id="harga" class="form-control input-style" placeholder="Isi Harga Satuan" value="<?php echo @$result->harga?>">
											</div>
										</div>
									</div>
								...
								<table class="table table-bordered table-hover mt-4 mb-4">
									...
									<tbody id="tabel_bbminuman">
										<?php include 'tampil_databbminuman2.php';?>
									</tbody>
								</table>
							</div>

							<div class="form-group text-center">
								<button type="button" name="ubah_minuman" id="ubah_minuman" class="btn btn-warning text-light mt-4"><i class="fa fa-pencil"></i>&nbsp;&nbsp;Ubah</button>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="modal fade modalDataBBMinuman" tabindex="-1" role="dialog" aria-labelledby="HapusDataBahanBaku" aria-hidden="true">
	...
	<script type="text/javascript">
		$(document).on('click', '#tambah_bahanbaku', function() {
			...
			if(kd_bahanbaku == '' ) {
				...
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url()?>/tambah_databbminuman.php',
					...
					success: function() {
						...
						$('#jumlah_dibutuhkan').val('');
						$('#kd_bahanbaku').focus();
						$('#tabel_bbminuman').load('<?php echo base_url()?>/tampil_databbminuman2.php?kd_produk=' + kd_produk, function() {
							calculate();
						});
					}
				});
				...
				$.ajax({
					...
					success: function(result) {
						if(result == 1) {
							...
							$('#tabel_bbminuman').load('<?php echo base_url()?>/tampil_databbminuman2.php?kd_produk=' + $('#kd_produk').val(), function() {
								calculate();
							});
						}
					}
		...
		function calculate() {
			var value1 = 0;
			$('#tabel_bbminuman tr').each(function() {
		...
		$(document).on('click', '#ubah_minuman', function() {
			var kd_produk = $('#kd_produk').val();
			...
			if(nama_produk == '') {
				...
				Toast.fire({
					icon: 'error',
					title: 'Tolong diisi Nama Minuman!'
				})
				$('#nama_produk').focus();
			} else if(harga == '') {
				const Toast = Swal.mixin({
				...
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url()?>/ubah_produkMinuman.php',
					data: ({'ubah_minuman' : true, 'kd_produk' : kd_produk, 'nama_produk' : nama_produk, 'kategori_produk' : kategori_produk, 'harga' : harga}),
					success: function() {
						Swal.fire({
							icon: 'success',
						  	title: 'Sukses!',
							...
							window.location = "<?php echo base_url()?>/produk_minuman.php";
						});
					}
				});
			}

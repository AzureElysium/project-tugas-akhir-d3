...
<head>
	...
	<title>DJ Resto | Kelola Produk - Ubah Makanan</title>
...
<body>
			...
			<div class="container-fluid content-top-gap">
				...
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb my-breadcrumb">
						...
						<li class="breadcrumb-item active" aria-current="page">Ubah Makanan</li>
					</ol>
				</nav>
				<!-- End Breadcrumb -->
							...
							<div class="card card_border border-top shadow p-3">
								<div class="row border-bottom">
									<div class="col-lg-6 border-right">
										...
										<div class="form-group row mb-4">
											...
											<div class="col-sm-8">
												<input type="text" name="nama_produk" id="nama_produk" class="form-control input-style" placeholder="Isi Nama Makanan" value="<?php echo @$result->nama_produk?>">
											</div>
										</div>
								...
								<table class="table table-bordered table-hover mt-4 mb-4">
									...
									<tbody id="tabel_bbmakanan">
										<?php include 'tampil_databbmakanan2.php';?>
									</tbody>
								</table>
							</div>

							<div class="form-group text-center">
								<button type="button" name="ubah_makanan" id="ubah_makanan" class="btn btn-warning text-light mt-4"><i class="fa fa-pencil"></i>&nbsp;&nbsp;Ubah</button>
							</div>
						</div>
					</div>
				</div>
	...
	<script type="text/javascript">
			...
			if(kd_bahanbaku == '' ) {
				...
				$.ajax({
					...
					success: function() {
						...
						$('#jumlah_dibutuhkan').val('');
						$('#kd_bahanbaku').focus();
						$('#tabel_bbmakanan').load('<?php echo base_url()?>/tampil_databbmakanan2.php?kd_produk=' + kd_produk, function() {
							calculate();
						});
					}
				});
				...
				$.ajax({
					...
					success: function(result) {
						if(result == 1) {
							...
							$('#tabel_bbmakanan').load('<?php echo base_url()?>/tampil_databbmakanan2.php?kd_produk=' + $('#kd_produk').val(), function() {
								calculate();
							});
						}
					}
		...
		$(document).on('click', '#ubah_makanan', function() {
			var kd_produk = $('#kd_produk').val();
			...
			if(nama_produk == '') {
				...
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url()?>/ubah_produkMakanan.php',
					data: ({'ubah_makanan' : true, 'kd_produk' : kd_produk, 'nama_produk' : nama_produk, 'kategori_produk' : kategori_produk, 'harga' : harga}),
					success: function() {
						Swal.fire({
							icon: 'success',
						  	title: 'Sukses!',
